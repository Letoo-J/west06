 ...
 * 图片加工工具类
 */
public class VerificationCodeAdapterUtil {
    private static final String IMAGE_PATH = "src/main/resources/static/image/";

    private static int ORI_WIDTH = 300;//源文件宽度
    private static int ORI_HEIGHT = 150;//源文件高度
    ...
    public static VerificationCodePlace getRandomVerificationCodePlace() {
        // 从文件夹中读取所有待选择文件
        ArrayList<String> imageFileNames = getFileNamesFromDic(IMAGE_PATH);
        // 随机获取
        int r = (int) Math.round(Math.random() * (imageFileNames.size() - 1));
        String imgName = imageFileNames.get(r);
        int[][] data = VerificationCodeAdapterUtil.getBlockData();

        return cutAndSave(IMAGE_PATH + imgName, data);
    }
    ...
    // 处理存放
    private static VerificationCodePlace cutAndSave(String path, int[][] data) {
        try {
            // 进行图片处理
            ...
            if (originImage != null) {
                ...
                ImageIO.write(originImage, "png", os);
                byte[] originImages = os.toByteArray();
                ByteArrayOutputStream osM = new ByteArrayOutputStream();
                ImageIO.write(markImage, "png", osM);
                byte[] markImages = osM.toByteArray();

                return new VerificationCodePlace(originImages, markImages, locationX, locationY);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
